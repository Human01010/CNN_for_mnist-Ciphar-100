...
class SimplifiedCNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)  # 第1层
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
        self.inception = InceptionBlock(64, 16)  # 第2-5层 (InceptionBlock 输出通道数为 16*4=64)
        self.layer1 = nn.Conv2d(64, 128, kernel_size=3, stride=2, padding=1, bias=False)  # 第6层
        self.bn2 = nn.BatchNorm2d(128)
        self.global_pool = nn.AdaptiveAvgPool2d(1)  # 第7层
        self.fc = nn.Linear(128, 100)  # 全连接层

    def forward(self, x):
        x = self.relu(self.bn1(self.conv1(x)))
        x = self.inception(x)
        x = self.relu(self.bn2(self.layer1(x)))
...
if __name__ == "__main__":
    ...
    # model1 = SimplifiedCNN().to(device)  # Current model
    #
    # convergence_epochs = [50]  # Replace with actual data
